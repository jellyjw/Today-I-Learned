...
_.take = function (arr, n) {
  ...
  let result = [];
  // if (n > arr.length) return (n = arr.length);
  if (n >= arr.length) {
    return arr;
  }
  for (let i = 0; i < n; i++) {
...
//   // TODO: 여기에 코드를 작성합니다.
//   if (n === undefined || n < 0) return arr.slice();
...
_.last = function (arr, n) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (n >= arr.length) {
      // n이 배열의 길이를 벗어날 경우, 전체 배열을 shallow copy한 새로운 배열을 리턴힌다.
      return arr;
    } else {
      // n의 범위인 경우 배열의 길이에서 n을 뺀 위치의 길이에서 배열의 길이까지 리턴한다.
...
_.reduce = function (arr, iteratee, initVal) {
  //initVal 초기값(누적이 되는 값)
  _.each(arr, (acc, cur) => {
    //initVal이 없고 인덱스가 0번째이면 처음에 초기값 넣어주기
    if (initVal === undefined && cur === 0) {
      initVal = acc;
    } else {
